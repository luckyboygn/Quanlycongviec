...
const EvaluationRepository = {
  async findByUserAndPeriod(userId, month, year) {
    return await db.getAsync(`
      SELECT e.*, u.full_name, u.position, u.role, u.department_id, d.name as department_name
      FROM evaluations e
    ...
    if (existing) {
      ...
      const res = await db.runAsync(`
        INSERT INTO evaluations (
          ...
          avg_score_total, notes, mgr_notes, deputy_notes, head_notes, status
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `, [
        ...
        finalAvg, data.notes || null, data.mgr_notes || null, data.deputy_notes || null, data.head_notes || null, data.status || 'draft'
      ]);
      return res.lastID;
    }
  },
